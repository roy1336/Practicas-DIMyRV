...
#define ARCHIVO_WRL 0
#define ARCHIVO_OBJ 1
...
public:
	...
	bool lectorArchivo(const char *ruta);
	void dibujaOBJ();
	void printOBJ();
};
bool OBJ::lectorArchivo(const char *ruta) {
	FILE *archivo;
	char linea[64];
...
bool WRL::lectorArchivo(const char *ruta) {
	FILE *archivo;
	char linea[64];
	vector<string> tokens;
	char keyword[64];
...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	if (fileExists) {
		if (tipoArchivo == ARCHIVO_OBJ) {
			objeto.dibujaOBJ();
		} else if(tipoArchivo == ARCHIVO_WRL) {
			mundo.dibujaWRL();
		}
	}
	//glPointSize(10.0);
...
void control_cb(int control) {
	...
	if (control == 7) {
		...
		extension = splitStr(file_name, '.');

		cout << extension.size() << endl;

		if (extension[1].compare("wrl")==0 || extension[1].compare("WRL") == 0) {
			tipoArchivo = ARCHIVO_WRL;
			fileExists = mundo.lectorArchivo(file_name.c_str());
		...
		else if (extension[1].compare("obj") == 0 || extension[1].compare("OBJ") == 0) {
			tipoArchivo = ARCHIVO_OBJ;
			fileExists = objeto.lectorArchivo(file_name.c_str());
			if (fileExists) {
				cout << "OBJ Cargado";
				//objeto.printOBJ();
			}
			...
			cout << "Formato de archivo seleccionado no válido" << endl;
			fileExists = false;
		}
		}
}
...
int main(int argc, char** argv)   // Main Function
{
	...
	main_window = glutCreateWindow("Practica 2: OBJ y WRL"); // Nombre de la Ventana
	...
	edit = GLUI_Master.create_glui_subwindow(main_window, GLUI_SUBWINDOW_LEFT);
